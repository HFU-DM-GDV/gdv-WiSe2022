...
img_object = cv2.drawKeypoints(
    img_obj_gray, keypoints_obj, img_object, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
)

img_table = cv2.drawKeypoints(
    img_table_gray, keypoints_table, img_table, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
)
...
img_matching = cv2.drawMatchesKnn(
    img_object, keypoints_obj, img_table, keypoints_table, good, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS
)
