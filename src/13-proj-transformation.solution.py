...
while True:
    ...
    if not (computationDone) and (len(ref_pt_src) == 4 and len(ref_pt_dst) == 4):
        T_perspective = cv2.getPerspectiveTransform(
            np.float32(ref_pt_src), np.float32(ref_pt_dst), solveMethod=cv2.DECOMP_SVD
        )
        print(
            "\nProjective transform:\n",
            "\n".join(["\t".join(["%03.3f" % cell for cell in row]) for row in T_perspective]),
        )
